<%@page import="com.github.drinkjava2.jwebbox.JspBox"%>
...
public static class ParameterUtil{ 
	public static Map getParameterMap(String parameterString) {
		...
		if (JspBox.isEmptyStr(parameterString))
			return m;
		String[] arr = parameterString.split("&");
		for (int i = 0; i < arr.length; i++) {
			String string = arr[i];
			String[] arr2 = string.split("=");
			if (arr2 != null && arr2.length == 2 && !JspBox.isEmptyStr(arr2[0])) {
				m.put(arr2[0], arr2[1]);
			}
		}
		return m;
	...
	public static String getParameterString(Map parameterMap) {
		...
		for (Iterator iter = set.iterator(); iter.hasNext();) {
			...
			if (!JspBox.isEmptyStr((String) parameterMap.get(key_))) {
				if (sb.length() > 0)
					sb.append("&");
...
<%
String paginId=JspBox.getAttribute(pageContext, "pageId");
int pageNo=(Integer)JspBox.getAttribute(pageContext, paginId+"_pageNo");  
int totalPage=(Integer)JspBox.getAttribute(pageContext, "totalPage");
